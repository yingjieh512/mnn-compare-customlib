...
After v17, a bounded v18r2 optimization attempt improved the custom performance candidate to `2.14021` decode TPS and `467.244 ms` TPOT on the same Device Farm S26 Ultra pool. That result is not accepted as the official final because the added v19r2 deterministic output-quality guard failed:

- Stock quality validation run: `arn:aws:devicefarm:us-west-2:884244642857:run:64d2cc31-abd6-49f8-97da-162f82410bc0/28a7b8ce-cb81-4e31-8498-c59164e9cdaa`, PASSED.
- Custom quality validation run: `arn:aws:devicefarm:us-west-2:884244642857:run:64d2cc31-abd6-49f8-97da-162f82410bc0/e8582532-a655-4619-b348-9c07b5740f3d`, FAILED.
- Custom path evidence in the failed quality run still reports `use_mnn_fallback = 0`, `calls_mnn_llm_response_for_measured_generation = false`, `full_custom_decode = true`, and `fallback_op_families = []`.
- Output correctness failed: exact token match `0 / 5`, comparison-gate pass `0 / 5`, and 4 of 5 custom prompt outputs were degenerate repeated token `220` sequences.

Therefore v18r2 is treated as a rejected experimental optimization. The official final report remains this v17 backend sweep, and no custom speedup or production-quality Qwen replacement is claimed.
